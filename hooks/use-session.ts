...
export interface Session {
  ConnectSession: (id: string) => Promise<string>;
  ...
  const ConnectSession = async (id: string) => {
    try {
      var isConnected: boolean = id
        ? connect(`${process.env.NEXT_PUBLIC_USE_HTTP == "true" ? "ws" : "wss"}://${process.env.NEXT_PUBLIC_GO_BACKEND}/session/connect?id=${id}`)
        : connect(`${process.env.NEXT_PUBLIC_USE_HTTP == "true" ? "ws" : "wss"}://${process.env.NEXT_PUBLIC_GO_BACKEND}/session/connect`);
