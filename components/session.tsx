...
// Hmm how does this get re-rendered if the session updates? i.e a new user joins
// I think the session context provider is responsible for updating the session state
// and then the session component listens to the session state and re-renders when it changes
export const Session: React.FC<SessionProps> = ({ children }) => {
  const { session } = useSessionContext();
  const linkURL = session.sessionState?.sessionId ? `${process.env.NEXT_PUBLIC_DOMAIN}?id=${session.sessionState?.sessionId}` : `${process.env.NEXT_PUBLIC_DOMAIN}`;
  ...
  const ConnectButton: React.FC = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center">
        <Button variant="default" onClick={async () => await session.ConnectSession(idQueryParam || "")}>
