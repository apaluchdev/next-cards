...
const CardViewer: React.FC<CardViewerProps> = ({ playingCards }) => {
  ...
  const GetPlayingCardImage: React.FC<{ playingCard: PlayingCard }> = ({
    playingCard,
  }) => {
    if (!playingCard) return <div></div>;

    if (playingCard.Suit == "Maybe")
      return (
        <Image
          className="pt-6 pb-6"
          src="/back-card.png"
      ...
      return (
        <div className="pt-6 pb-6">
          <Card
            className={`w-16 h-24  hover:border-red-500 hover:cursor-pointer hover:transform hover:-translate-y-1 transition duration-300 ease-in-out`}
          >
